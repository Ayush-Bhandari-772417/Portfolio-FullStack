// admin\src\components\contentpage\SocialMediasContent.tsx
"use client";
...
export default function SocialMediasContent() {
  const {
    loading,
    error,
    showForm,
    ...
    return (
      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-xl font-bold mb-6">
          {currentItem ? "Edit Social Media" : "Add Social Media"}
        </h2>
        <SocialMediaForm
          socialmedia={currentItem}
      ...
      {loading ? (
        ...
        currentItems.length === 0 ? (
          <div className="text-center py-8 text-gray-500">
            {searchQuery ? 'No matching social medias found.' : 'No social medias yet. Add your first social media!'}
          </div>
        ) : (
          <>
            {/* Table */}
                  ...
                  {currentItems.map((s) => {
                    return (
                      <tr key={s.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{s.url}</td>
